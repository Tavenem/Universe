...
{
    /// <summary>
    /// Converts a <see cref="StarSystem"/> to and from JSON.
    /// </summary>
    public class StarSystemConverter : JsonConverter
    {
        /// <summary>
        ...
        /// </returns>
        public override bool CanConvert(Type objectType) => objectType == typeof(StarSystem);
        ...
        public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
        {
            ...
            writer.WritePropertyName(nameof(StarSystem.StarType));
            ...
            serializer.Serialize(writer, location.Shape.ContainingRadius, typeof(Number));

            writer.WritePropertyName("Mass");
            serializer.Serialize(writer, location.Mass, typeof(Number));

            writer.WritePropertyName(nameof(StarSystem.StarIDs));
            writer.WriteStartArray();
            foreach (var item in location.StarIDs)
            {
        ...
        internal object FromJObj(JObject jObj)
        {
            ...
            if (!jObj.TryGetValue(nameof(StarSystem.StarType), out var starTypeToken)
            ...
            Number radius;
            if (!jObj.TryGetValue("Radius", out var radiusToken)
                || radiusToken.Type != JTokenType.String)
            {
                throw new JsonException();
            }
            else
            {
                radius = radiusToken.ToObject<Number>();
            }

            Number mass;
            if (!jObj.TryGetValue("Mass", out var massToken)
                || massToken.Type != JTokenType.String)
            {
                throw new JsonException();
            }
            else
            {
                mass = massToken.ToObject<Number>();
            }
            ...
            var starIds = starIdsArray.ToObject<IReadOnlyList<string>>();
            if (starIds is null)
            {
                throw new JsonException();
            }
