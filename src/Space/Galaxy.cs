...
namespace Tavenem.Universe.Space
{
    public partial class CosmicLocation
    {
        ...
        private static readonly List<ChildDefinition> _GalaxyChildDefinitions = new()
        {
            ...
            new ChildDefinition(BlackHole.BlackHoleSpace, _GalaxySystemDensity * new HugeNumber(4, -4), CosmicStructureType.BlackHole),
        ...
        private void ReconstituteGalaxyInstance(Vector3 position, double? temperature)
        {
            ...
            {
                radius = randomizer.NextNumber(new HugeNumber(2.4, 20), new HugeNumber(2.5, 21)); // 25000–75000 ly
                axis = radius * randomizer.NormalDistributionSample(0.02, 0.001);
            }
            else if (StructureType == CosmicStructureType.EllipticalGalaxy)
            {
                radius = randomizer.NextNumber(new HugeNumber(1.5, 18), new HugeNumber(1.5, 21)); // ~160–160000 ly
                axis = radius * randomizer.NormalDistributionSample(0.5, 1);
            }
            else if (StructureType == CosmicStructureType.DwarfGalaxy)
            {
                radius = randomizer.NextNumber(new HugeNumber(9.5, 18), new HugeNumber(2.5, 18)); // ~200–1800 ly
                axis = radius * randomizer.NormalDistributionSample(0.02, 1);
            }
            else
            {
                radius = randomizer.NextNumber(new HugeNumber(1.55, 19), new HugeNumber(1.55, 21)); // ~1600–160000 ly
                axis = radius * randomizer.NormalDistributionSample(0.02, 0.001);
            }
