    ...
    public partial class CosmicLocation : Location
    {
        ...
        public async Task<(CosmicLocation? child, List<CosmicLocation> subChildren)> GenerateChildNearAsync(IDataStore dataStore, ChildDefinition condition, Vector3 position)
        {
                ...
                && defs.Sum(x => x.rem).IsPositive)
            {
                ...
                var idealDistance = (double)(2 * (def.def.Density * HugeNumber.ThreeQuartersPI).CubeRoot());
        ...
        public HugeNumber GetRadiusWithChildren(int maxAmount, ChildDefinition? condition = null)
        {
            ...
            return (3 * v / HugeNumber.FourPI).CubeRoot();
        }
        ...
        internal HugeNumber GetRocheLimit(HugeNumber orbitingDensity)
            => new HugeNumber(8947, -4) * (Mass / orbitingDensity).CubeRoot();
