...
namespace Tavenem.Universe.Space
{
    ...
    public struct Orbit : ISerializable, IEquatable<Orbit>
    {
        ...
        private Orbit(SerializationInfo info, StreamingContext context) : this(
            (HugeNumber?)info.GetValue(nameof(OrbitedMass), typeof(HugeNumber)) ?? default,
            ...
            (HugeNumber?)info.GetValue(nameof(MeanMotion), typeof(HugeNumber)) ?? default,
            (HugeNumber?)info.GetValue(nameof(Periapsis), typeof(HugeNumber)) ?? default,
            (Vector3?)info.GetValue(nameof(R0), typeof(Vector3)) ?? default,
            (HugeNumber?)info.GetValue(nameof(Radius), typeof(HugeNumber)) ?? default,
            (HugeNumber?)info.GetValue(nameof(SemiMajorAxis), typeof(HugeNumber)) ?? default,
            (HugeNumber?)info.GetValue(nameof(StandardGravitationalParameter), typeof(HugeNumber)) ?? default,
            (double?)info.GetValue(nameof(TrueAnomaly), typeof(double)) ?? default,
            (Vector3?)info.GetValue(nameof(V0), typeof(Vector3)) ?? default,
            (HugeNumber?)info.GetValue(nameof(Period), typeof(HugeNumber)) ?? default,
        ...
        /// </param>
        /// <returns>A change of velocity vector.</returns>
        public static Vector3 GetDeltaVForCircularOrbit(CosmicLocation orbitingObject, HugeNumber orbitedMass, Vector3 orbitedPosition)
        {
        ...
        {
            var meanAnomaly = MeanMotion * t % HugeNumber.TwoPI;
            return ((double)((MeanLongitude + meanAnomaly) % HugeNumber.TwoPI), meanAnomaly);
        }
        ...
        public OrbitalParameters GetOrbitalParameters()
        {
            ...
            var angleAscending = Vector3.UnitX.Angle(xz) - HugeNumber.HalfPI;
        ...
        /// vector.</returns>
        public (Vector3 position, Vector3 velocity) GetStateVectorsAtTime(HugeNumber t)
        {
        ...
        public double GetTrueAnomalyAtTime(HugeNumber t)
        {
            ...
            return (double)HugeNumber.Atan2((p / StandardGravitationalParameter).Sqrt() * Vector3.Dot(r, r), p - Radius);
        }
            ...
            Vector3 orbitedPosition)
        {
            ...
            var longitudeAscending = Vector3.UnitX.Angle(xz) - HugeNumber.HalfPI;
            ...
            var argPeriapsis = HugeNumber.Acos(Vector3.Dot(n, r0) / (n.Length() * radius));
            if (r0.Z < 0)
            {
                argPeriapsis = HugeNumber.TwoPI - argPeriapsis;
            }
            ...
            var period = HugeNumber.TwoPI * HugeNumber.Sqrt(semiMajorAxis.Cube() / standardGravitationalParameter);
            ...
            double trueAnomaly)
        {
            ...
            while (meanAnomaly < 0)
            {
                meanAnomaly += HugeNumber.TwoPI;
            }
            var period = HugeNumber.TwoPI * HugeNumber.Sqrt(semiMajorAxis.Cube() / standardGravitationalParameter);
            var meanMotion = period.IsZero ? HugeNumber.Zero : HugeNumber.TwoPI / period;
            var epoch = Duration.FromSeconds(meanMotion.IsZero ? HugeNumber.Zero : meanAnomaly / meanMotion);
        ...
        internal static HugeNumber GetSemiMajorAxisForPeriod(HugeNumber orbitingMass, HugeNumber orbitedMass, HugeNumber period)
            => (HugeNumberConstants.G * (orbitingMass + orbitedMass) * period * period / (HugeNumber.FourPI * HugeNumber.PI)).CubeRoot();
