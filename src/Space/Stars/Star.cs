...
namespace Tavenem.Universe.Space
{
    ...
    public class Star : CosmicLocation
    {
        ...
        private HugeNumber GetMass(Randomizer randomizer)
        {
            if (StarType == StarType.BrownDwarf)
            {
                return randomizer.NextNumber(new HugeNumber(2.468, 28), new HugeNumber(1.7088, 29));
            }
            if (StarType == StarType.WhiteDwarf)
            {
            ...
            if (StarType == StarType.RedGiant)
            {
                ...
                {
                    return randomizer.NextNumber(new HugeNumber(1.592, 31), new HugeNumber(4.975, 31)); // Super/hypergiants
                }
                else
                {
                    return randomizer.NextNumber(new HugeNumber(5.97, 29), new HugeNumber(1.592, 31)); // (Bright)giants
                }
            }
            if (StarType == StarType.YellowGiant)
            {
                if (LuminosityClass == LuminosityClass.Zero)
                {
                    return randomizer.NextNumber(new HugeNumber(1, 31), new HugeNumber(8.96, 31)); // Hypergiants
                }
                else if (LuminosityClass is LuminosityClass.Ia
                    or LuminosityClass.Ib)
                {
                    return randomizer.NextNumber(new HugeNumber(5.97, 31), new HugeNumber(6.97, 31)); // Supergiants
                }
                else
                {
                    return randomizer.NextNumber(new HugeNumber(5.97, 29), new HugeNumber(1.592, 31)); // (Bright)giants
            ...
            if (StarType == StarType.BlueGiant)
            {
                if (LuminosityClass == LuminosityClass.Zero) // Hypergiants
                {
                    ...
                    or LuminosityClass.Ib)
                {
                    return randomizer.NextNumber(new HugeNumber(9.95, 30), new HugeNumber(2.0895, 32)); // Supergiants
                }
                else
                {
                    return randomizer.NextNumber(new HugeNumber(3.98, 30), new HugeNumber(1.99, 31)); // (Bright)giants
                }
            }
        ...
        private void Reconstitute(Vector3 position, double temperature)
        {
            ...
            if (StarType == StarType.BrownDwarf)
            {
                ...
                var flattening = randomizer.NextNumber(HugeNumber.Deci);
            ...
            else if (StarType == StarType.WhiteDwarf)
            {
                ...
                var radius = (new HugeNumber(1.8986, 27) / mass).CubeRoot() * 69911000;
                var flattening = (HugeNumber)randomizer.NormalDistributionSample(0.15, 0.05, minimum: 0);
                shape = new Ellipsoid(radius, radius * (1 - flattening), position);
